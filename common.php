...
if (!function_exists('get_img_url')) {
    ...
    function get_img_url($id = 0)
    {
        $path = model('admin/attachment')->getFilePath($id);
        if (!$path) {
            $image_path= config('uploads_img_url').config('public_static_path').'admin/img/none.png';
        } else{
            $image_path=config('uploads_img_url').  $path;
        }
        return $image_path;
    }
}
...
if (!function_exists('module_config')) {
     ...
     * @param string $name
     * @param string $value
     * @author caiweiming <xxxxxxxx@qq.com>
     * @return mixed
     */
...
if (!function_exists('get_nickname')) {
    ...
    function get_nickname($uid = 0)
    {
        ...
        if (isset($list[$key])) {
            ...
            // 调用接口获取用户信息
            $info = model('user/user')->field('nickname')->where('user_id',$uid)->find();
...
if (!function_exists("get_group_info")) {
    ...
    {
        return UserGroup::where('group_id',$group_id)->find();

        $group_info=cache('get_group_info_'.$group_id);
        if(!$group_info)
        {
            $group_info=UserGroup::where('group_id',$group_id)->find();
            cache('get_group_info_'.$group_id, $group_info, 3600);
        }
        return $group_info;
    }
 ...
 * 字符串隐藏隐藏
 */
if(!function_exists('formatStrHide')){
    function formatStrHide($str, $start, $end){
        //截取银行卡号前4位
...
function httpGets($url, $data =null){
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
...
function HttpGet($url, $data=[],$timeout = 60){
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "GET");
...
function getGoogleCode(){
    ...
    $qrCodeUrl = $ga->getQRCodeGoogleUrl('FreeArk', $secret);
